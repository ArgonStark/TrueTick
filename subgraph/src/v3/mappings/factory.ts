...
export function handlePoolCreated(event: PoolCreated): void {
  ...
  // Slipstream's PoolCreated carries tickSpacing, not fee, so feeTier cannot be
  // read from the event as it is on Uniswap v3. Read the swap fee from the pool
  // contract instead -- tickSpacing is NOT a fee and substituting it would be
  // badly wrong: the live NVDAc/USDC pool has tickSpacing 10 but fee() 500,
  // and swap.ts divides feeTier by 1e6 to compute feesUSD, so tickSpacing would
  // understate fees 50x. Reading fee() also matches what backfill.ts already
  // does in this repo.
  // try_ so a reverting pool cannot halt indexing; feeTier 0 only zeroes
  // feesUSD, leaving price, TVL and volume intact.
  const poolContract = PoolABI.bind(event.params.pool)
  const feeResult = poolContract.try_fee()
  if (feeResult.reverted) {
    log.warning('fee() reverted for Slipstream pool {}; feeTier set to 0', [event.params.pool.toHexString()])
    pool.feeTier = ZERO_BI
  } else {
    pool.feeTier = BigInt.fromI32(feeResult.value)
  }
